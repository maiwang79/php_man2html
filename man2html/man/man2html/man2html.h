...
#include <ctype.h>
#include <sys/stat.h>
#include "man/man2html/defs.h"
#include "man/man2html/strdefs.h"
#include "man/man2html/cgibase.h"
//#include "man/src/version.h"
...
out_html(char *c) {
	...
	if (scaninbuff) {
		...
		while (*c) {
                        if (*c != '\a')
                               returnbuffer[buffpos++] = *c;
                               if (buffpos >= buffmax){
                                   xrealloc(returnbuffer,buffmax *2);
                                   buffmax = buffmax *2;
                               }
...
xrealloc(void *ptr, size_t size) {
     ...
     if (p == NULL){ printf("alloc memory fail %i",size);
	  error_page("Out of memory3",
			 "Sorry, out of memory, aborting...\n");
     }
     return p;
}
